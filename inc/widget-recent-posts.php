...
if(!class_exists('LQThemesPosts_Widget')){
 class LQThemesPosts_Widget extends WP_Widget {
    
	public function __construct() {
		
		parent::__construct(
			'lqthemes_recent_posts', // Base ID
			__( 'LQThemes: Recent Posts', 'lqthemes-companion' ), // Name
			array( 'description' => __( 'Recent Posts.', 'lqthemes-companion' ), ) // Args
		);
		
	}
 	function form( $instance ) {
 	    $defaults = array('list_num' => '4','style' => '1', 'title' => __( 'Recent Posts', 'lqthemes-companion' ));
...
<p>
  <label for="<?php echo $this->get_field_id( 'title' ); ?>">
    <?php _e('Title', 'lqthemes-companion'); ?>
    :</label>
  <br />
...
<p>
  <label for="<?php echo $this->get_field_id( 'list_num' ); ?>">
    <?php _e('Recent Posts List Num', 'lqthemes-companion'); ?>
    :</label>
  <br />
...
<?php
	...
	function widget( $args, $instance ) {
 		extract( $args );
 	    $title    = apply_filters(__('Recent Posts', 'lqthemes-companion'), esc_attr($instance['title']) );
...
<div class="widget-project">
  <div class="row">
   ...
   if ( has_post_thumbnail() ) {
         $featured_image = wp_get_attachment_image_src(get_post_thumbnail_id(), 'lqthemes-widget-post-image');
		 $source = get_site_url();
		 if($featured_image[0] !=""){
...
function lqthemes_companion_recent_posts(){
	
	register_widget('LQThemesPosts_Widget');
						
}
add_action( 'widgets_init', 'lqthemes_companion_recent_posts' );
