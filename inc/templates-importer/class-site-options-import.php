 ...
 * @since  1.0.0
 */
class LQThemes_Site_Options_Import {

	/**
	 * Instance of LQThemes_Site_Options_Importer
	 */
	private static $_instance = null;

	/**
	 * Instanciate LQThemes_Site_Options_Importer
	 */
		...
		if ( isset( $cats ) ) {
			...
			foreach ( $cats as $key => $cat ) {
				...
				if ( ! empty( $cat['slug'] ) && ! empty( $cat['thumbnail_src'] ) ) {
					...
					$image = (object) LQThemes_Sites_Helper::_sideload_image( $cat['thumbnail_src'] );
	...
	private function insert_logo( $image_url = '' ) {
		...
		$data = (object) LQThemes_Sites_Helper::_sideload_image( $image_url );
