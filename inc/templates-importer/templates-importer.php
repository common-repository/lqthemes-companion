...
if (!class_exists('LQThemesTemplater')){
class LQThemesTemplater {

	public function __construct($atts = NULL)
		{
			add_action( 'wp_ajax_lqthemes_import_elementor', array(&$this ,'import_elementor'));
			add_action( 'wp_ajax_nopriv_lqthemes_import_elementor', array(&$this ,'import_elementor'));
	...
	public static function templates(){
		...
		$feed_url = 'http://lqthemes.com/templates-rss/?license_key='.$license_key.'&theme='.$theme;
		...
		if ( $maxitems == 0 ) :
			...
			$required_plugins = array( 'elementor' => array( 'name' => __( 'Elementor Page Builder', 'lqthemes-companion' ) ) );
			$plugin = '';
			$my_plugins = '';
		...
		return apply_filters( 'lqthemes_elementor_templates_list', $templates_list );
	...
	public static function render_admin_page() {
		...
		$templates_array = LQThemesTemplater::templates();
	...
	public static function get_button_html( $slug, $file='' ) {
		...
		$state  = LQThemesTemplater::check_plugin_state( $slug, $file );
		if ( ! empty( $slug ) ) {
			switch ( $state ) {
				case 'install':
					$nonce  = wp_nonce_url(
					...
					$button .= '<a data-slug="' . $slug . '" class="install-now lqthemes-install-plugin button button-primary" href="' . esc_url( $nonce ) . '" data-name="' . $slug . '" aria-label="Install ' . $slug . '">' . __( 'Install and activate', 'lqthemes-companion' ) . '</a>';
					break;
				case 'activate':
					...
					$button             .= '<a data-slug="' . $slug . '" class="activate-now button button-primary" href="' . esc_url( $nonce ) . '" aria-label="Activate ' . $slug . '">' . __( 'Activate', 'lqthemes-companion' ) . '</a>';
					break;
			}// End switch().
		}// End if().
		return $button;
	...
	public function import_elementor($template='', $template_url='', $ajax = true, $set_as_home = false ) {
		...
		$templates_list = LQThemesTemplater::templates();
